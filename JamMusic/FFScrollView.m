...
-(id)initPageWithFrame:(CGRect)frame urls:(NSMutableArray *)urls{
    self = [super initWithFrame:frame];
    if (self) {
        selectionType = FFScrollViewSelecttionTypeTap;
        sourceArr = [urls copy];
        self.userInteractionEnabled = YES;
        [self initSubViewsWithFrame:frame WithUrl:urls];
    }
    return self;
}
-(void)initSubViewsWithFrame:(CGRect )frame WithUrl:(NSMutableArray *)urls{
    ...
    for (int i = 0; i < [urls count]; i++) {
        ...
        AFNetworkReachabilityManager *manager = [AFNetworkReachabilityManager sharedManager];
         if ([manager isReachable]) {
             [imageview sd_setImageWithURL:[urls objectAtIndex:i] placeholderImage:[UIImage imageNamed:@"0002"] options:SDWebImageAvoidAutoSetImage completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
